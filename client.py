...
def scan_and_probe_ports(host: str) -> Dict[int, str]:
    ...
    # Prioritize known and common ports for speed
    known_ports = [80, 443, 8080, 8888, 25565] # Common chat/game ports
    common_ports = list(range(1, 1024))
    full_scan_ports = list(range(1024, 65536))
    
    scan_ports = list(dict.fromkeys(known_ports + common_ports + full_scan_ports)) # Remove duplicates
    ...
    try:
        ...
        if sorted_results:
            ...
            console.print(f"[yellow]No joinable chat servers found in the scanned port range.[/yellow]")
        ...
        return {}
def get_os_from_ip(ip_address: str, perform_os_scan: bool) -> str:
    """
    Performs an Nmap OS detection scan on a given IP address with a 15-second timeout.

    Args:
        ip_address (str): The IP address to scan.
        perform_os_scan (bool): Whether to perform the OS scan.

    Returns:
        str: The detected OS, or "Unknown" if detection fails, is skipped, or times out.
    """
    if not perform_os_scan:
        return "Unknown"
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
        try:
            with console.status(f"[cyan]Scanning OS for {ip_address}...[/cyan]", spinner="dots"):
                future = executor.submit(scan_worker)
                result = future.result(timeout=15)
                return result
        except concurrent.futures.TimeoutError:
            console.log(f"[yellow]OS scan for {ip_address} timed out after 15 seconds.[/yellow]")
            return "Unknown (Timeout)"
        except nmap.nmap.PortScannerError as e:
            if "requires root privileges" in str(e).lower():
                console.log("[bold red]Nmap OS detection requires root/administrator privileges.[/bold red]")
            else:
                console.log(f"[bold red]Nmap error for {ip_address}: {e}[/bold red]")
            return "Unknown"
        except Exception as e:
